...
def main(
    ...
    # New image loading and resizing logic
    init_image_pil = Image.open(args.source_img_dir).convert('RGB')
    fixed_size = 1024  # Define the target size for both width and height
    init_image_pil = init_image_pil.resize((fixed_size, fixed_size), Image.Resampling.LANCZOS)
    init_image = np.array(init_image_pil) # HWC numpy array

    # After resizing, init_image.shape is (fixed_size, fixed_size, 3)
    height = init_image.shape[0] # This will be fixed_size (1024)
    width = init_image.shape[1]  # This will be fixed_size (1024)

    # The init_image (HWC numpy array) is passed to encode
    ...
    info['target_age'] = args.target_age
    info['age_low'] = 30
    info['age_high'] = 70
